<template>
          ...
          <div class="modal-body member-details">
            ...
            <div class="detail-row">
              ...
              <div class="detail-value">
                <img v-if="selectedMember.id_card_copy" :src="selectedMember.id_card_copy" alt="สำเนาบัตรประชาชน" class="document-preview">
              </div>
            </div>
            
            <div class="detail-row">
              <div class="detail-label">สำเนาทะเบียนบ้าน:</div>
              <div class="detail-value"><img v-if="selectedMember.house_registration_copy" :src="selectedMember.house_registration_copy" alt="" class="document-preview">
              </div>
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      // จัดการการอัพโหลดรูปภาพของบัตรประชาชน
async handleIdCardUpload(event) {
  ...
  if (file) {
    ...
    try {
      ...
      this.formData.id_card_copy = response.data.fileUrl;

      console.log('ID card image uploaded successfully');
      
      // บันทึกชื่อไฟล์ลงในฐานข้อมูล
      if (this.formData.member_id) {
        await this.saveFileNameToDatabase(this.formData.member_id, 'id_card_copy', response.data.fileUrl);
      }

      // บันทึกลงใน localStorage เพื่อให้ข้อมูลไม่หายไป
      localStorage.setItem(`id_card_${this.formData.member_id}`, response.data.fileUrl);
    } catch (error) {
...
// จัดการการอัพโหลดรูปภาพทะเบียนบ้าน
async handleHouseRegUpload(event) {
  const file = event.target.files[0];
  if (file) {
    // ตรวจสอบขนาดไฟล์ (ไม่เกิน 5MB)
    if (file.size > 5 * 1024 * 1024) {
    ...
    try {
      ...
      // อัปโหลดไฟล์
      const response = await api.uploadFile(formData);
      this.formData.house_registration_copy = response.data.fileUrl;

      console.log('House registration image uploaded successfully');
      
      // บันทึกชื่อไฟล์ลงในฐานข้อมูล
      if (this.formData.member_id) {
        await this.saveFileNameToDatabase(this.formData.member_id, 'house_registration_copy', response.data.fileUrl);
      }

      // บันทึกลงใน localStorage เพื่อให้ข้อมูลไม่หายไป
      localStorage.setItem(`house_reg_${this.formData.member_id}`, response.data.fileUrl);
    } catch (error) {
      console.error('Error uploading file:', error);
      alert('ไม่สามารถอัพโหลดไฟล์ได้');
    }
